...
private:
  ...
  void laser_callback(const sensor_msgs::msg::LaserScan::SharedPtr _scan_msg) {
    ...
    for (auto i = 0u; i < _scan_msg->ranges.size(); ++i) {
      ...
      if (i >= static_cast<unsigned>(_scan_msg->ranges.size() * 4 / 8) &&
               i < static_cast<unsigned>(_scan_msg->ranges.size() * 6 / 8)) {
      ...
      else if (i >= static_cast<unsigned>(_scan_msg->ranges.size() * 6 / 8) &&
               i < static_cast<unsigned>(_scan_msg->ranges.size() * 7 / 8)) {
        // get the distance reading to the closest object in the given zone
      ...
      else if (i >= static_cast<unsigned>(_scan_msg->ranges.size() * 7 / 8) ||
               i < static_cast<unsigned>(_scan_msg->ranges.size()  / 8)) {
        // get the distance reading to the closest object in the given zone
      ...
      else if (i >= static_cast<unsigned>(_scan_msg->ranges.size() * 1 / 8) &&
               i < static_cast<unsigned>(_scan_msg->ranges.size() * 2 / 8)) {
        // get the distance reading to the closest object in the given zone
      ...
      else if (i >= static_cast<unsigned>(_scan_msg->ranges.size() * 2 / 8) &&
               i <= static_cast<unsigned>(_scan_msg->ranges.size() * 4 / 8)) {
        // get the distance reading to the closest object in the given zone
  ...
  */
  void set_drive_logic_state() {
    ...
    if (z[0] > d && z[1] > d && z[2] > d && z[3] > d && z[4] > d) {
      ...
      drive_logic_state = 0; // turn left
    } else if (z[0] > d && z[1] > d && z[2] > d && z[3] < d && z[4] > d) {
      RCLCPP_DEBUG(this->get_logger(),
                   "Case 4: obstacle only in front-left zone");
      drive_logic_state = 0; // turn right
    } else if (z[0] > d && z[1] < d && z[2] < d && z[3] > d && z[4] > d) {
      RCLCPP_DEBUG(this->get_logger(),
    ...
    // logic block 2:
      ...
      drive_logic_state = 2; // keep moving straight ahead, obstacle only in right zone
    } else if (z[0] < d && z[1] > d && z[2] < d && z[3] > d && z[4] > d) {
      RCLCPP_DEBUG(this->get_logger(),
    ...
    // logic block 3:
      ...
      drive_logic_state = 1; // find obstacle: turn CW and move ahead
    } else if (z[0] > d && z[1] < d && z[2] > d && z[3] > d && z[4] < d) {
      RCLCPP_DEBUG(this->get_logger(),
      ...
      drive_logic_state = 5; // turn right // find obstacle: turn CW and move ahead
    } else if (z[0] > d && z[1] > d && z[2] < d && z[3] < d && z[4] < d) {
      RCLCPP_DEBUG(this->get_logger(),
      ...
      drive_logic_state = 5; // turn right
    } else if (z[0] > d && z[1] < d && z[2] > d && z[3] < d && z[4] < d) {
      RCLCPP_DEBUG(
          this->get_logger(),
    ...
    // logic block 4:
      ...
      drive_logic_state = 0; // turn right // find obstacle: turn CW and move ahead
    } else if (z[0] < d && z[1] < d && z[2] < d && z[3] > d && z[4] < d) {
      RCLCPP_DEBUG(
          this->get_logger(),
      ...
      drive_logic_state = 1; // turn left // reverse turning left
    } else if (z[0] < d && z[1] < d && z[2] > d && z[3] < d && z[4] < d) {
      RCLCPP_DEBUG(
          this->get_logger(),
  ...
  */
  void determine_vel_msg() {
    ...
    case 5:
      // Turn right: obstacle in  front-left
      vel_msg.linear.x = 0.0;
      vel_msg.angular.z = -angular_z_velocity_;
      RCLCPP_DEBUG(this->get_logger(),
                   "Behaviour 5: Turn right, obstacle in front-left  "
                   );
