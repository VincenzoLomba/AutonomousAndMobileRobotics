...
class SimpleMoveITPlannerPoseGoalNode(Node):

    def __init__(self):
        
        super().__init__('simple_moveit_planner_posegoal_node')
...
def main(args=None):
    ...
    node = SimpleMoveITPlannerPoseGoalNode()
    ...
    node.get_logger().info('Threaded executor started! Now sending MoveIt2 pose goal command...')

    # Actually moving to a desired pose goal thanks to MoveIt2:
    position = [0.7, -0.2, 0.7]
    quat_xyzw = [1,0,0, 0]
    node.moveit2.move_to_pose(position=position, quat_xyzw=quat_xyzw) # Send the command to MoveIt2 to move the end-effector
