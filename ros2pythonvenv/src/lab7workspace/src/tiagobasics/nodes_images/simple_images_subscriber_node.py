...
class SimpleImagesSubscriberNode(Node):
    ...
    def callback_camera_image(self, msg):
        self.get_logger().info('Image received from camera_topic! Now parsing it...')
        self.img = self.bridge.imgmsg_to_cv2(msg, desired_encoding="bgr8")
        cv2.imshow("TiagoCameraImage", self.img) # Show the image using OpenCV in a new window called "TiagoCameraImage"
...
def main():
    ...
    try:
        # Keep processing callbacks (and OpenCV GUI) until Ctrl+C
